...
db.business.aggregate([
	{"$group": {"_id":"$country", "number of biz in each country": {"$sum": 1}}},
	{"$match": {"_id": "NV"}}
]);
...
/*  $project dung de print nhung element minh muon
...
db.business.aggregate( [ 
	{"$group": {"_id": {"country": "$country"}, "busisesses": {"$sum": 1} } },
    {"$project": {"_id": 1, "Average number of businesses": {"$avg": "$businesses"} } }
] );
...
]); /*$unwind to break the array reviews into single objects*/
...
db.business.aggregate([
	...
	{ "$group": { "_id": {"id": "$_id", "name": "$name"}, 
					"average stars per business": { "$avg": "$reviews.review_score" } } }
]); 
